...
const outputPath = join(projectRoot, 'tests', '.tmp', 'v4-poc-packed.json')

export async function packV4PocFixtures() {
  ...
  for (const fixture of manifest.fixtures) {
    ...
    packed.push({ id: fixture.id, archivePath, archiveBytes: archive.size })
  }
...
function run(command, args) {
  return new Promise((resolvePromise, reject) => {
    ...
    child.once('exit', (code) => {
      ...
      reject(new Error(`V4 POC fixture pack failed with exit code ${code}`))
    })
  })
}
